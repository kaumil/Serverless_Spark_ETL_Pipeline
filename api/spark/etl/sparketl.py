...
temp = df.withColumn(
    "Occurrence Category_merged", transform_udf(col("Occurrence Category"))
)
print(temp.head(10))
...
print(
    temp.select(
        col("Occurrence Date:"),
        month(to_date(col("Occurrence Date:"), "yyyy-MM-dd")).alias("month"),
    ).show()
)
...
temp = temp.withColumn(
    "GR", when(col("Province:") != "", col("Province:")).otherwise(col("TC Region:"))
)
...
print(
    temp.select(
        [count(when(col(c).contains("None"), c)).alias(c) for c in temp.columns]
    ).show()
)
print(
    temp.select(
        [count(when(col(c).contains("NULL"), c)).alias(c) for c in temp.columns]
    ).show()
)
print(temp.select([count(when(col(c) == "", c)).alias(c) for c in temp.columns]).show())
print(
    temp.select([count(when(col(c).isNull(), c)).alias(c) for c in temp.columns]).show()
)
print(temp.select([count(when(isnan(c), c)).alias(c) for c in temp.columns]).show())
...
temp = temp.withColumn(
    "hour", hour(to_timestamp(substring(col("Occurrence Time:"), 1, 4), "HHmm"))
)
temp = temp.withColumn(
    "timestamp", to_timestamp(substring(col("Occurrence Time:"), 1, 4), "HHmm")
)
...
featureset2 = aircraft_information1.groupBy("CADORS Number").agg(
    collect_list("flight_rule").alias("flight_rule"),
    collect_list("model").alias("model"),
...
featureset2 = (
    ...
    .withColumn("model_merged", transform_udf(col("model")))
...
temp = temp.select(
    [when(col(c) == "", "NA").otherwise(col(c)).alias(c) for c in temp.columns]
)
print(temp.head(2))
print(temp.select([count(when(col(c) == "", c)).alias(c) for c in temp.columns]).show())
print(
    temp.select([count(when(col(c) == "NA", c)).alias(c) for c in temp.columns]).show()
)
...
raw_data = spark.read.json("s3://cadorscmpt732/processed_data/")
...
df = df.withColumn(
    "label", when(col("Fatalities:") + col("Injuries:") > 1, 1).otherwise(0)
)

# Making Aircraft Dataframe
af = df.select(df["CADORS Number"], df["Aircraft Information"]).cache()
af = (
    af.withColumn("Flight Rule", af["Aircraft Information"]["Flight Rule:"])
    .withColumn("Model", af["Aircraft Information"]["Model:"])
    .withColumn("Registration Mark", af["Aircraft Information"]["Registration Mark:"])
    .withColumn("Aircraft Category", af["Aircraft Information"]["Aircraft Category:"])
    .withColumn("Make", af["Aircraft Information"]["Make:"])
    .withColumn("Year Built", af["Aircraft Information"]["Year Built:"])
    .withColumn("Amateur Built", af["Aircraft Information"]["Amateur Built:"])
    .withColumn("Gear Type", af["Aircraft Information"]["Gear Type:"])
    .withColumn("Phase of Flight", af["Aircraft Information"]["Phase of Flight:"])
    .withColumn("Damage", af["Aircraft Information"]["Damage:"])
    .withColumn("Operator Type", af["Aircraft Information"]["Operator Type:"])
)
...
aircraft_information = (
    ai1.join(ai2, ["pos", "CADORS Number"])
    .join(ai5, ["pos", "CADORS Number"])
    .join(ai6, ["pos", "CADORS Number"])
    .join(ai8, ["pos", "CADORS Number"])
    .join(ai10, ["pos", "CADORS Number"])
    .join(ai11, ["pos", "CADORS Number"])
    .join(ai15, ["pos", "CADORS Number"])
    .join(ai16, ["pos", "CADORS Number"])
    .join(ai17, ["pos", "CADORS Number"])
    .join(ai19, ["pos", "CADORS Number"])
)
# print(aircraft_information.show())
aircraft_information1 = aircraft_information.toDF(
    "pos",
    "CADORS Number",
    "registration_mark",
    "model",
...
).join(aircraft_information1, ["CADORS Number"], how="left")
print(aircraft_df.show(5))

# print(aircraft_df.count())
# print(aircraft_df.dropDuplicates().count())
aircraft_df.write.json("s3://cadorscmpt732/processed_data/")

aircraft_df = aircraft_df.withColumn(
    "year", year(to_date(col("Occurrence Date:"), "yyyy-MM-dd"))
)
# aircraft_df.dtypes
...
# data1.show()
data1.write.json("s3://cadorscmpt732/processed_data/")
